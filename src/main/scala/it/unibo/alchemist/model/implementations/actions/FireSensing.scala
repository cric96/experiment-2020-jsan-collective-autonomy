...
import it.unibo.alchemist.model.implementations.nodes.{FireNode, SimpleNodeManager}
...
case class FireSensing[T, P <: Position[P]](node: Node[T], env: Environment[T, P], baseTemperatureMolecule: String) extends AbstractAction[T](node) {
  ...
  override def execute(): Unit = {
    ...
    val fireIntensity: Double = if (fire.isEmpty) {
      0.0
    } else {
      fire.maxBy(_.intensity(env.getPosition(node))).intensity(env.getPosition(node))
    }
