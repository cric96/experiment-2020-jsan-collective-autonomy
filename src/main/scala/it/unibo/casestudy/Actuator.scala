...
object Actuator {
  def removeIfPresent(nodeManager: NodeManager, molecule: String) : Unit = if(nodeManager.has(molecule)) {
    nodeManager.remove(molecule)
  }
  def act(nodeManager: NodeManager, actuation: Actuation) : Unit = actuation match {
    case NoActuation => removeIfPresent(nodeManager, "target")
